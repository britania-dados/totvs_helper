...
def filter_pentaho_fields(
    fields: List[FieldMeta], include_free_fields: bool
) -> List[FieldMeta]:
    return filter_fields(fields, include_free_fields)


def stream_field_names(
    filtered_fields: List[FieldMeta], partition_field: str
) -> List[str]:
    return [field.name for field in filtered_fields] + [partition_field]
...
def sync_value_field_names(
    filtered_fields: List[FieldMeta],
    partition_field: str,
    *,
...
def _pentaho_meta(
    field: FieldMeta | str, *, partition: bool = False
) -> _PentahoMeta:
    if partition:
        name = str(field)
        return "String", 8 if name == "BASE" else 2, -1, ""
    assert isinstance(field, FieldMeta)
    if field.data_type == "character":
        return "String", field.width, -1, ""
    ...
    if field.data_type in ("decimal", "numeric"):
        return "BigNumber", max(field.width, 17), field.decimals, (
            "######0.0###################;-######0.0###################"
        )
    return "String", field.width or 255, -1, ""
...
def _build_row_meta(
    ...
    *,
    ...
    parts = [
        _build_value_meta(field.name, _pentaho_meta(field), origin=origin, stage=stage)
